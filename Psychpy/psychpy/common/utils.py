...
def wandb_true_pred_scatters(run, y_test, y_pred, path, specifier,):
    ...
    px.scatter(np.arange(len(y_test)), y_test,
                alpha=0.7, marker='+', label='True')

    px.scatter(np.arange(len(y_pred)), y_pred,
...
def print_the_evaluated_results(results, learning_method, ):
    ...
    for repeat, result in results.items():
        ...
        if learning_method == "regression":
            ...
            MEAPE.append(mean_estimation_absolute_percentage_error(y_true=y_true, y_pred=y_pred))
        ...
        if learning_method == "classification":
            ...
            if len(np.unique(y_true)) <= 2:  # binary
                ROC_AUC.append(
                    metrics.roc_auc_score(
                        y_true=y_true, y_score=y_pred_prob.max(axis=1), average="weighted",
                    )
                )
            else:  # multiclass
                enc = OneHotEncoder(sparse=False, )
                y_true_ = enc.fit_transform(y_true.reshape(-1, 1), )

                ROC_AUC.append(
                    metrics.roc_auc_score(
                        y_true=y_true_, y_score=y_pred_prob, average="weighted", multi_class="ovr"
                    )
                )
            ...
            diff = len(tnr) - len(support)
            if diff > 0:  # making support dim equal to
                support = np.append(support, np.repeat(0, diff))
            ...
            if y_pred_prob is not None:
                ROC_AUC.append(
                    metrics.roc_auc_score(
                        y_true, y_pred_prob, average='weighted',
                    ),
                )
            ...
            ROC_AUC.append(np.nan)  # appending an impossible outcome of ROC_AUC to avoid adding one more

    if learning_method == "regression":
        MEA = np.nan_to_num(np.asarray(MEA))
        RMSE = np.nan_to_num(np.asarray(RMSE))
        MEAPE = np.nan_to_num(np.asarray(MEAPE))
        ...
        meape_ave = np.mean(MEAPE, axis=0)
        meape_std = np.std(MEAPE, axis=0)

        r2_ave = np.mean(R2_Score, axis=0)
        r2_std = np.std(R2_Score, axis=0)

        print(
            f"   mae ", "   rmse ", "\t meape", "\t r2_score ",
        )
        ...
        print(
            ...
            # f" Ave ± std \t Ave ± std \t Ave ± std \t Ave ± std \t Ave ± std \t Ave ± std \t Ave ± std \t Ave ± std"
        )

        print(
            f"   Precision \t Recall   \t F1-score  \t ROC_AUC  \t Accuracy  \t TNR \n"
            f"{precision_ave:.3f} ± {precision_std:.3f} \t"
            f"{recall_ave:.3f} ± {recall_std:.3f} \t"
            f"{f1_score_ave:.3f} ± {f1_score_std:.3f} \t"
            f"{roc_auc_ave:.3f} ± {roc_auv_std:.3f} \t"
            f"{acc_ave:.3f} ± {acc_std:.3f} \t"
            f"{tnr_ave:.3f} ± {tnr_std:.3f} \t"
            f"\n"
        )
